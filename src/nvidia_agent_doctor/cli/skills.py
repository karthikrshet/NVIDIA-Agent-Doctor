...
def scan(
    ...
    if json_output:
            ...
            output.append(
                {
                    ...
                    "findings": [
                        redact_data({**f, "severity": f["severity"].value}) for f in r.findings
                    ],
                }
            )
        if risk_graph:
            ...
            typer.echo(
                json.dumps(
                    redact_data({"skills": output, "risk_graph": graph.to_dict()}),
                    indent=2,
                )
            )
        else:
        ...
        if not results:
            ...
            for result in results:
                ...
                table.add_row(
                    redact_data(result.skill.name),
                    redact_data(result.skill.path),
            ...
            if verbose:
                for result in results:
                    console.print(
                        f"\n[bold]{redact_data(result.skill.name)}[/bold] "
                        f"({redact_data(result.skill.path)})"
                    )
            ...
            if risk_graph:
                ...
                console.print(redact_data(graph.render_ascii()))
    ...
    _exit_for_results(results)


def _exit_for_results(results: Sequence[SkillScanResult]) -> None:
    """Map heuristic skill severities to the documented diagnostic exit codes."""
    levels = {result.risk_level for result in results}
    if SecuritySeverity.HIGH in levels or SecuritySeverity.CRITICAL in levels:
        raise typer.Exit(code=3)
    if SecuritySeverity.MEDIUM in levels or SecuritySeverity.LOW in levels:
        raise typer.Exit(code=1)
