...
def doctor(
    ...
    fix: bool = typer.Option(
        False, "--fix",
        help="Suggest safe remediation steps (requires confirmation)."
    ),
...
def _run_doctor(
    ...
    verbose: bool = False,
    quiet: bool = False,
) -> "DiagnosticReport":  # type: ignore[name-defined]  # noqa: F821
    ...
    from nvidia_agent_doctor.core.result import DiagnosticReport, SectionResult, CheckResult
    ...
    _add_optional_section(
        report, "tensorrt", "TensorRT",
        lambda: _tensorrt_section(check_tensorrt()), quiet, console
    )
    _add_optional_section(
        report, "triton", "Triton",
        lambda: _triton_section(check_triton()), quiet, console
    )
    _add_optional_section(
        report, "openshell", "OpenShell",
        lambda: _openshell_section(detect_openshell()), quiet, console
    )
    _add_optional_section(
        report, "nemotron", "Nemotron",
        lambda: _nemotron_section(detect_nemotron()), quiet, console
    )
...
def _add_optional_section(report, name, display, fn, quiet, console) -> None:
    from nvidia_agent_doctor.core.result import SectionResult, CheckResult
    from nvidia_agent_doctor.core.severity import Severity
...
def _tensorrt_section(info: dict) -> "SectionResult":  # noqa: F821
    from nvidia_agent_doctor.core.result import SectionResult, CheckResult
    from nvidia_agent_doctor.core.severity import Severity
...
def _triton_section(info: dict) -> "SectionResult":  # noqa: F821
    from nvidia_agent_doctor.core.result import SectionResult, CheckResult
    from nvidia_agent_doctor.core.severity import Severity
...
def _openshell_section(info: dict) -> "SectionResult":  # noqa: F821
    from nvidia_agent_doctor.core.result import SectionResult, CheckResult
    from nvidia_agent_doctor.core.severity import Severity
...
def _nemotron_section(info: dict) -> "SectionResult":  # noqa: F821
    from nvidia_agent_doctor.core.result import SectionResult, CheckResult
    from nvidia_agent_doctor.core.severity import Severity
...
def _show_fix_suggestions(report, console: Console) -> None:
    ...
    for check in fixable:
        ...
        if confirmed:
            console.print("  [yellow]Manual application required — copy and run the command above.[/yellow]")
